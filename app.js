document.addEventListener("DOMContentLoaded", function () {
  ...
  const infoBox = document.querySelector(".toggle-info-box");

  if (toggleButton && infoBox) {
    toggleButton.addEventListener("click", () => {
      infoBox.classList.toggle("hidden");

      // (Opcional) Cambiar el texto del botón al hacer toggle
      const isVisible = !infoBox.classList.contains("hidden");
      toggleButton.textContent = isVisible
        ? "🔽 Ocultar información sobre vigilancia e inteligencia competitiva"
        : "📘 ¿Quieres saber más sobre vigilancia e inteligencia competitiva?";
    });
  }
});
